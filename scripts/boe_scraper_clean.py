...
def get_folders():
    ...
    try:
        ...
        if data.get('success'):
            ...
            print(f"ERR Failed to get folders: {data.get('errorMessage')}")
            return []
    except Exception as e:
        print(f"ERR Error fetching folders: {e}")
        return []
...
def get_laws_in_folder(folder_id, folder_name):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if laws:
                        print(f"OK Found {len(laws)} laws via {endpoint}")
                        return laws
                except:
    ...
    print(f"WARN  Could not fetch laws for {folder_name}")
    return []
