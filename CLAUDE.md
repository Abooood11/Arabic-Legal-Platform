...
| التاريخ | الملخص |
|---------|--------|
| 2026-02-17 | ربط إحالات المواد الخارجية: "المادة X من نظام Y" → رابط أزرق لصفحة النظام + كاش مكتبة + إصلاح السادسة عشرة + المادة الجافة |
...
### 8.8 إحالات خارجية خاطئة — "المادة X من نظام Y" (2026-02-17)
- **المشكلة:** "المادة الرابعة من نظام المعالجات التجارية" تُنشئ رابطاً خاطئاً للمادة 4 من النظام **الحالي** بدل النظام المُشار إليه
- **السبب:** لا يوجد كشف للإحالات الخارجية — أي "المادة X" يُعامل كإحالة داخلية
- **الحل:**
  - `EXTERNAL_LAW_PATTERN`: regex لكشف "من نظام|لائحة|تنظيم|قانون|مرسوم|قرار" بعد رقم المادة
  - `SELF_REFERENCE_PATTERN`: استثناء "من هذا النظام" (إحالة ذاتية → يبقى الرابط)
  - `external_reference` segment type جديد مع `externalLawName`
  - كاش المكتبة (`loadLibraryCache`) يُحمّل مرة واحدة ويُشارك بين كل instances
  - `findLawByName` بحث ذكي (مطابقة تامة → جزئية → بدون النوع)
  - عرض الرابط الخارجي بلون أزرق + أيقونة `ExternalLink` + تلميح باسم النظام
- **الملفات:** `ArticleReferenceText.tsx`

### 8.9 السادسة عشرة تُكتشف كـ السادسة (2026-02-17)
- **المشكلة:** progressive matching يتحقق من `validArticleNumbers` في كل خطوة — "السادسة"=6 موجودة فيتوقف قبل "السادسة عشرة"=16
- **الحل:** مرحلتان: Phase 1 يجد الأطول (بغض النظر عن الصحة)، Phase 2 يتحقق من النهائي فقط

### 8.10 "المادة الجافة" false positive (2026-02-17)
- **المشكلة:** "المادة" بمعنى substance (مثل "المادة الجافة") تُعامل كإحالة قانونية
- **الحل:** `NON_ARTICLE_FOLLOWERS` set مع كلمات المواد الكيميائية/الفيزيائية (الجافة، الفعالة، الخام...)
